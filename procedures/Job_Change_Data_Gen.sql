...
-- Job_Change_Data_Gen procedure variables with v_ prefix
...
execute immediate ' select MIN(CHANGE_ID) from JOB_CHANGE_DATA ' into c_Change_ID;
...
if ( v_Error_Random = 1)  then
  ...
  execute immediate 'update JOB_CHANGE_DATA set ' || mycolumNames(v_Null_Ran) || ' = null where CHANGE_ID = ' || c_Change_ID ;
  v_Tab.delete(1);
  v_Test_Var := v_Test_Var + 1;
-- Invalid date promised
elsif ( v_Error_Random = 2 ) then
  c_Date_Promised := getErrorTimeID(20,60);
  execute immediate 'update JOB_CHANGE_DATA set ' || mycolumNames(6) || ' = '|| c_Date_Promised || ' where CHANGE_ID = ' || c_Change_ID ;
  v_Tab.delete(2);
  v_Test_Var := v_Test_Var + 1;
-- Invalid date shipped
elsif ( v_Error_Random = 3) then 
   c_Date_Ship_By := getErrorTimeID(20,60);
   execute immediate 'update JOB_CHANGE_DATA set ' || mycolumNames(7) || ' = '|| c_Date_Ship_By || ' where CHANGE_ID = ' || c_Change_ID ;
   v_Tab.delete(3);
   v_Test_Var := v_Test_Var + 1;
-- Invalid Location_Id
   ...
   c_Location_ID := c_Location_ID + trunc( dbms_random.value(100,1000.99) ) ;
    execute immediate 'update JOB_CHANGE_DATA set ' || mycolumNames(4) || ' = '|| c_Location_ID || ' where CHANGE_ID = ' || c_Change_ID ;
   v_Tab.delete(4);
   v_Test_Var := v_Test_Var + 1;   
-- Invalid Cust_Id
   ...
   c_Cust_ID_Ordered_By := c_Cust_ID_Ordered_By + trunc( dbms_random.value(100,1000.99) ) ;
    execute immediate 'update JOB_CHANGE_DATA set ' || mycolumNames(5) || ' = '|| c_Cust_ID_Ordered_By || ' where CHANGE_ID = ' || c_Change_ID ;
   v_Tab.delete(5);
   v_Test_Var := v_Test_Var + 1;
-- Invalid Sales_Agent_Id
   ...
   execute immediate 'update JOB_CHANGE_DATA set ' || mycolumNames(2) || ' = '|| c_Sales_Agent_ID || ' where CHANGE_ID = ' || c_Change_ID ;
   v_Tab.delete(6);
   v_Test_Var := v_Test_Var + 1;   
-- Invalid Sales_Class_Id
   ...
   execute immediate 'update JOB_CHANGE_DATA set ' || mycolumNames(3) || ' = '|| c_Sales_Class_ID || ' where CHANGE_ID = ' || c_Change_ID ;
   v_Tab.delete(7);
   v_Test_Var := v_Test_Var + 1;   
--  Invalid Lead_Id
   ...
   execute immediate 'update JOB_CHANGE_DATA set ' || mycolumNames(12) || ' = '|| c_Lead_ID || ' where CHANGE_ID = ' || c_Change_ID ;
   v_Tab.delete(8);
   v_Test_Var := v_Test_Var + 1;   
-- Invalid contract date
elsif ( v_Error_Random = 9 ) then
  c_Contract_Date := getErrorTimeID(20,60);
  execute immediate 'update JOB_CHANGE_DATA set ' || mycolumNames(1) || ' = '|| c_Contract_Date || ' where CHANGE_ID = ' || c_Change_ID ;
  v_Tab.delete(9);
  v_Test_Var := v_Test_Var + 1;
else
...
execute immediate ' select MIN(CHANGE_ID) from JOB_CHANGE_DATA where CHANGE_ID > '|| c_Change_ID  into c_Change_ID;
end loop;
